    ...
    class per_item_packing{
            ...
            {                      $this->flatrate[$rule_no]=array();
              ...
              function get_Rates_From_Api()
              {
                ...
                $respone_code   = wp_remote_retrieve_response_code($response);

                if( $respone_code != 200 ) {
                    $message = ! empty($response['response']['message']) ? $response['response']['message'] : null;
                    $this->debug("<pre> Response Code: ".$respone_code."<br/> Response Message: ". $message );
                    return false;
                }
                ...
                foreach($this->flatrate as $f_pids)
                {
                    ...
                    {
                        $cost+=($this->product_quantity[$f_pid] * (float) $f_rate );
                    }
                }
                return $cost;
              }
